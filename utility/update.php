...
if($_GET['from']) {
	if(md5($_GET['from'].$_G['config']['security']['authkey']) != $_GET['frommd5']) {
		...
		if(!$operation || !$version || !$release) {
			show_msg('Forum của bạn không đúng version mà chúng tôi yêu cầu.');
		}
		$time = $_G['timestamp'];
...
if(file_exists($lockfile) && !$_GET['from']) {
	show_msg('Bạn hãy đăng nhập vào máy chủ ftp chứa dữ liệu và xóa file ./data/update.lock để bắt đầu việc update.');
}
...
if(!file_exists($sqlfile)) {
	show_msg('File SQL '.$sqlfile.' không tồn tại, hãy kiểm tra lại.');
}
...
if($_POST['delsubmit']) {
	...
	show_msg('Đã xóa thành công table và field.', $theurl.'?step=style');
}
...
if($_GET['step'] == 'start') {
	...
	if(!$devmode && !C::t('common_setting')->fetch('bbclosed')) {
		...
		updatecache('setting');
		show_msg('Forum của bạn chưa được đóng cửa, chúng tôi sẽ tự động đóng cửa xin chờ trong giây lát....', $theurl.'?step=start', 5000);
	}
	if(version_compare($version, '1.5.2') <= 0) {
		show_msg('Bạn phải nâng cấp UCenter lên version 1.6.0 mới có thể update được。<br>Vui lòng tải UCenter 1.6.0 tại forum support sau đó xóa toàn bộ dữ liệu trong ./uc_server chỉ trừ file ./uc_server/data/config.inc.php rồi chạy chương trình update để nâng cấp.');
	} else {
		show_msg('Mô tả <br> Quá trình nâng cấp sẽ dùng file install.sql mới nhất, để đồng bộ hóa và nâng cấp dữ liệu。<br>
			Hãy chắc file ./data/install.sql sử dụng phiên bản mới nhất.<br><br>
			Trước khi nâng cấp vui lòng đóng tất cả các Plugin đang hoạt động để đảm bảo quá trình nâng cấp điễn ra tốt hơn.<br><br>
			<a href="'.$theurl.'?step=prepare'.($_GET['from'] ? '&from='.rawurlencode($_GET['from']).'&frommd5='.rawurlencode($_GET['frommd5']) : '').'">Bất đầu quá trình Update.</a>');
	}
	...
	if(empty($list) && empty($_GET['sendsql'])) {
		...
		$msg = 'Dữ liệu đang được nâng cấp, xin chờ trong giây lát......';
		$notice = '<br><br><b>Kết quả nâng cấp cơ sở dữ liệu:</b><br>'.$list.base64_decode($_GET['sendsql']);
	...
	if(DB::fetch_first("SHOW COLUMNS FROM ".DB::table('forum_activityapply')." LIKE 'contact'")) {
		$query = DB::query("UPDATE ".DB::table('forum_activityapply')." SET message=CONCAT_WS(' Liên hệ:', message, contact) WHERE contact<>''");
	...
	if(!$row = DB::fetch_first("SHOW COLUMNS FROM ".DB::table('forum_forumfield')." LIKE 'seodescription'")) {
		DB::query("ALTER TABLE ".DB::table('forum_forumfield')." ADD seodescription text NOT NULL default '' COMMENT 'SEO' AFTER keywords");
	...
	show_msg('Bắt đầu nâng cấp cấu trúc dữ liệu.', $theurl.'?step=sql');
	...
	if(empty($newtables) || empty($newsqls)) {
		show_msg('Bạn có chắc tập tin SQL rỗng.');
	}

	$i = empty($_GET['i'])?0:intval($_GET['i']);
	$count_i = count($newtables);
	if($i>=$count_i) {
		show_msg('Nâng cấp cấu trúc dữ liệu thành công.Chuẩn bị bước tiếp theo...', $theurl.'?step=data');
	}
	...
	if(!$query = DB::query("SHOW CREATE TABLE ".DB::table($newtable), 'SILENT')) {
		...
		if(!DB::query($usql, 'SILENT')) {
			show_msg('Quá trình thêm bảng dữ liệu '.DB::table($newtable).'gặp lỗi hãy thực hiện các câu lệnh sau để chạy lại quá trình update. Những file lỗi:<br><br>'.dhtmlspecialchars($usql));
		} else {
			$msg = 'Quá trình thêm bảng dữ liệu '.DB::table($newtable).' thành công.';
		}
	} else {
			...
			if($key == 'PRIMARY') {
				if($value != $oldcols[$key]) {
					if(!empty($oldcols[$key])) {
						...
						if(!DB::query($usql, 'SILENT')) {
							show_msg('Quá trình thêm bảng dữ liệu '.DB::table($newtable).' gặp lỗi trong lúc nâng cấp bảng dữ liệu, xin thực hiện các lệnh sau để quá trình update bất đầu lại.<br><br><b>Câu lệnh nâng cấp bảng dữ liệu</b>:<div style=\"position:absolute;font-size:11px;font-family:verdana,arial;background:#EBEBEB;padding:0.5em;\">'.dhtmlspecialchars($usql)."</div><br><b>Error</b>: ".DB::error()."<br><b>Errno.</b>: ".DB::errno());
						} else {
							$msg = 'Quá trình thêm bảng dữ liệu '.DB::table($newtable).' thành công!';
		...
		if(!empty($updates)) {
			...
			if(!DB::query($usql, 'SILENT')) {
				show_msg('nâng cấp bảng '.DB::table($newtable).'gặp lỗi trong lúc nâng cấp bảng dữ liệu, xin thực hiện các lệnh sau để quá trình update bất đầu lại.<br><br><b>Câu lệnh nâng cấp bảng dữ liệu</b>:<div style=\"position:absolute;font-size:11px;font-family:verdana,arial;background:#EBEBEB;padding:0.5em;\">'.dhtmlspecialchars($usql)."</div><br><b>Error</b>: ".DB::error()."<br><b>Errno.</b>: ".DB::errno());
			} else {
				$msg = 'nâng cấp bảng '.DB::table($newtable).' thành công!';
			}
		} else {
			$msg = 'Kiểm tra bảng dữ liệu '.DB::table($newtable).' nâng cấp hoàn tất.';
		}
	}
	...
	if(empty($_GET['op']) || $_GET['op'] == 'realname') {
		...
		if($i==0) {
			...
			if(!empty($value)) {
				show_msg("Thực hiện tính năng update hoàn thành.", "$theurl?step=data&op=$nextop");
			}
		...
		if($n>0) {
			show_msg("Thực hiện tính năng [$n/$t]", "$theurl?step=data&op=realname&i=$n");
		} else {
			show_msg("Thực hiện tính năng update hoàn thành.", "$theurl?step=data&op=$nextop");
		}
		...
		if(!$value) {
			DB::query("INSERT INTO ".DB::table('common_member_profile_setting')." VALUES ('birthdist', 1, 0, 0, 'Quận', 'Quận/Huyện', 0, 0, 0, 0, 0, 0, 0, 'select', 0, '', '')");
			DB::query("INSERT INTO ".DB::table('common_member_profile_setting')." VALUES ('birthcommunity', 1, 0, 0, 'Huyện', '', 0, 0, 0, 0, 0, 0, 0, 'select', 0, '', '')");
			DB::query("UPDATE ".DB::table('common_member_profile_setting')." SET title='nơi sinh' WHERE fieldid = 'birthcity'");
			DB::query("UPDATE ".DB::table('common_member_profile_setting')." SET title='nơi sống' WHERE fieldid = 'residecity'");
		}
		...
		if($profile['title'] == 'Ngày sinh') {
			DB::query("UPDATE ".DB::table('common_member_profile_setting')." SET title='Birthday' WHERE fieldid = 'birthday'");
			DB::query("UPDATE ".DB::table('common_member_profile_setting')." SET title='ID' WHERE fieldid = 'idcardtype'");
		...
		show_msg("Nâng cấp xong bảng thông tin tài khoản.", "$theurl?step=data&op=$nextop");
		...
		if(!isset($settings['focus'])) {
			$focusnew = array('title' => 'đề nghị', 'cookie' => 1);
		...
		if(!isset($settings['article_tags'])) {
			$article_tagsnew = array(1 => 'Original', 2 => 'Hot spot', 3 => 'Photos', 4 => 'Break news', 5 => 'Headlines', 6 => 'Magic lantern', 7 => 'Scroll', 8 => 'Recommend');
			$newsettings['article_tags'] = $article_tagsnew;
		}
		if(empty($settings['anonymoustext'])) {
			$newsettings['anonymoustext'] = 'Ẩn danh';
		}
		...
		if(!isset($settings['userreasons'])) {
			$newsettings['userreasons'] = 'Bạn cần phải!\r\n thiết lập điện toán đám may cho forum\r\!\r\\r\n';
		}
		...
		show_msg("Nâng cấp cấu hình", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp trang quản lý", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp phân loại thông tin", "$theurl?step=data&op=$nextop");
	} elseif($_GET['op'] == 'updatecron') {
		$nextop = 'updatemagic';
		if(!DB::result_first("SELECT filename FROM ".DB::table('common_cron')." WHERE filename='cron_cleanfeed.php'")) {
			DB::query("INSERT INTO ".DB::table('common_cron')." VALUES ('', '1','system','Tự động xóa dữ liệu không cần thiết.','cron_cleanfeed.php','1269746634','1269792000','-1','-1','0','0')");
		}

		if(!DB::result_first("SELECT filename FROM ".DB::table('common_cron')." WHERE filename='cron_checkpatch_daily.php'")) {
			DB::query("INSERT INTO ".DB::table('common_cron')." VALUES ('', '1','system','Tự động Update các bảng vá lỗi hằng ngày'','cron_checkpatch_daily.php','1269746639','1269792000','-1','-1','2','22')");
		}

		if(!DB::result_first("SELECT filename FROM ".DB::table('common_cron')." WHERE filename='cron_publish_halfhourly.php'")) {
			DB::query("INSERT INTO ".DB::table('common_cron')." VALUES ('', '1','system','Tự động thiết lập chủ đề','cron_publish_halfhourly.php','1269746639','1269792000','-1','-1','-1','0	30')");
		}

		if(!DB::result_first("SELECT filename FROM ".DB::table('common_cron')." WHERE filename='cron_follow_daily.php'")) {
			DB::query("INSERT INTO ".DB::table('common_cron')." VALUES ('','1','system','Tự động theo dõi chủ đề','cron_follow_daily.php','1269746639','1269792000','-1','-1','02','0')");
		}
		if(!DB::result_first("SELECT filename FROM ".DB::table('common_cron')." WHERE filename='cron_todayviews_daily.php'")) {
			DB::query("INSERT INTO ".DB::table('common_cron')." VALUES ('','1','system','Tự động xem bài mới của hôm nay','cron_todayviews_daily.php','1321500558','1321556400','-1','-1','3','0	5	10	15	20	25	30	35	40	45	50	55')");
		}
		if(!DB::result_first("SELECT filename FROM ".DB::table('common_cron')." WHERE filename='cron_member_optimize_daily.php'")) {
			DB::query("INSERT INTO ".DB::table('common_cron')." VALUES ('','0','system','Tự động xem bài mới của hôm nay','cron_member_optimize_daily.php','1321500558','1321556400','-1','-1','2','0	5	10	15	20	25	30	35	40	45	50	55')");
		}
		...
		if(!DB::result_first("SELECT filename FROM ".DB::table('common_cron')." WHERE filename='cron_todayheats_daily.php'")) {
			DB::query("INSERT INTO ".DB::table('common_cron')." VALUES ('','1','system','Tự động tối ưu hóa bảng','cron_todayheats_daily.php','1269746623','1269792000','-1','-1','0','0')");
		}

		show_msg("Nâng cấp dữ liệu đạo cụ", "$theurl?step=data&op=$nextop");
	} elseif($_GET['op'] == 'updatemagic') {
		$nextop = 'updatereport';
		if(DB::result_first("SELECT name FROM ".DB::table('common_magic')." WHERE identifier='highlight'")) {
			DB::query("UPDATE ".DB::table('common_magic')." SET name='Thẻ cầu vòng', description='Dùng để thay đổi màu sắc bài viết của bạn.' WHERE identifier='highlight'");
		}
		if(DB::result_first("SELECT name FROM ".DB::table('common_magic')." WHERE identifier='namepost'")) {
			DB::query("UPDATE ".DB::table('common_magic')." SET name='Thẻ xem người ẩn danh', description='Dùng để biết được ai đang ẩn danh.' WHERE identifier='namepost'");
		}
		if(DB::result_first("SELECT name FROM ".DB::table('common_magic')." WHERE identifier='anonymouspost'")) {
			DB::query("UPDATE ".DB::table('common_magic')." SET name='Thẻ ẩn danh', description='Dùng để ẩn danh bạn khi đăng bài viết.' WHERE identifier='anonymouspost'");
		}

		show_msg("Nâng cấp dữ liệu đạo cụ hoàn tất.", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu thông báo", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu thông báo hoàn tất", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu forum", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu forum hoàn tất.", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu diễn đàn lớn", "$theurl?step=data&op=$nextop");
		...
		DB::update('common_credit_rule', array('rulename' => 'Quy định'), "rulename='Đăng nhập mỗi ngày'");
		$count = DB::result_first("SELECT COUNT(*) FROM ".DB::table('common_credit_rule')." WHERE action='portalcomment'");
		if(!$count) {
			DB::query("INSERT INTO ".DB::table('common_credit_rule')." (`rulename`, `action`, `cycletype`, `cycletime`, `rewardnum`, `norepeat`, `extcredits1`, `extcredits2`, `extcredits3`, `extcredits4`, `extcredits5`, `extcredits6`, `extcredits7`, `extcredits8`, `fids`) VALUES ('portalcomment','portalcomment','1','0','40','1','0','1','0','0','0','0','0','0','')");
		}
		$count = DB::result_first("SELECT COUNT(*) FROM ".DB::table('common_credit_rule')." WHERE action='followedcollection'");
		if(!$count) {
			DB::query("INSERT INTO ".DB::table('common_credit_rule')." (`rulename`, `action`, `cycletype`, `cycletime`, `rewardnum`, `norepeat`, `extcredits1`, `extcredits2`, `extcredits3`, `extcredits4`, `extcredits5`, `extcredits6`, `extcredits7`, `extcredits8`, `fids`) VALUES ('Follow','followedcollection','1','0','3','0','0','1','0','0','0','0','0','0','')");
		}

		show_msg("Nâng cấp dữ liệu BBcode", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu BBcode hoàn tất", "$theurl?step=data&op=$nextop");
		...
		if(!DB::result_first("SELECT COUNT(*) FROM ".DB::table('common_smiley')." WHERE url='010.gif'")) {
			DB::query("REPLACE INTO ".DB::table('common_smiley')." (typeid, displayorder, type, code, url) VALUES ('4','19','stamp','Tem','010.gif')");
		}
		if(!DB::result_first("SELECT COUNT(*) FROM ".DB::table('common_smiley')." WHERE url='010.small.gif'")) {
			DB::query("REPLACE INTO ".DB::table('common_smiley')." (typeid, displayorder, type, code, url) VALUES ('0','18','stamplist','Danh sách tem','010.small.gif')");
		}
		if(!DB::result_first("SELECT COUNT(*) FROM ".DB::table('common_smiley')." WHERE url='011.small.gif'")) {
			DB::query("REPLACE INTO ".DB::table('common_smiley')." (typeid, displayorder, type, code, url) VALUES ('0','20','stamplist','Bài viết','011.small.gif')");
			$setnewbie = true;
		}
		require_once libfile('function/cache');
		updatecache('stamps');
		...
		show_msg("Nâng cấp dữ liêu đóng đấu bài viết.", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu quyền hạn", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu quyền hạn portal", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu trả lời portal", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu trả lời portal hoàn tất", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu bài viết portal", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu nhóm", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu nhóm hoàn tất", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu nhóm người dùng ", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu nhóm người dùng hoàn tất", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu thiết lập", "$theurl?step=data&op=$nextop");

	} elseif($_GET['op'] == 'pm') {
		$nextop = 'allowgetimage';
			DB::query("UPDATE ".DB::table('common_member')." SET newpm='0', newprompt='0'");
		show_msg("Nâng cấp dữ liệu thiết lập hoàn tất.", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu quyền hạn đính kèm wget hoàn tất", "$theurl?step=data&op=$nextop");
		...
		if($updateverify) {
			...
			if($t) {
				...
				if($n) {
					show_msg("Tên thật xác thực nâng cấp [$n/$t]", "$theurl?step=data&op=verify&i=$n&updateverify=true");
				}
			}
		}
		show_msg("Xác thực nâng cấp dữ liệu được hoàn thành", "$theurl?step=data&op=$nextop");
		...
		if($needupgrade && $count) {
			...
			if(DB::num_rows($query)) {
				...
				$start += $limit;
				show_msg("Đang xác thực dữ liệu ... ... $start/$count", "$theurl?step=data&op=forumattach&start=$start");
			}
		...
		show_msg("Nâng cấp dữ liệu đính kèm hoàn tất", "$theurl?step=data&op=$nextop");
	} elseif($_GET['op'] == 'forumstatlog') {
		$nextop = 'moderate';
		DB::query('DELETE FROM '.DB::table('forum_statlog')." WHERE logdate='0000-00-00'");
		show_msg("Nâng cấp dữ liệu đăng nhập hoàn tất", "$theurl?step=data&op=$nextop");
		...
		if($needupgraded) {
			...
			if(!file_exists($cachefile)) {
				...
				if($data && @$fp = fopen($cachefile, 'w')) {
					...
					show_msg("Nâng cấp bảng chủ đề", "$theurl?step=data&op=$nextop");
				}
			} else {
				$data = @file($cachefile);
				if(!$data) {
			...
			if(!$tids) {
				@unlink($cachefile);
				show_msg("Nâng cấp cache", "$theurl?step=data&op=$nextop");
			}
			$insertsql = array();
			...
			$start += $limit;
			show_msg("Đang xác thực dữ liệu ... ... $start ", "$theurl?step=data&op=threadimage&start=$start");
		} else {
			show_msg("Nâng cấp dữ liệu ảnh chủ đề", "$theurl?step=data&op=$nextop");
		}
		...
		show_msg("Nâng cấp dữ liệu trả lời trang home", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu người dùng", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu thông tin chủ đề hoàn tất", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu Plugin hoàn tất", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu quan tâm chủ đề hoàn tất", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu huy chương của thành viên hoàn tất", "$theurl?step=data&op=$nextop");
		...
		show_msg("Nâng cấp dữ liệu thiết lập độ dài ký tự hoàn tất", "$theurl?step=data&op=$nextop");
	} else {
		...
		if(save_config_file($configfile, $_config, $default_config, $deletevar)) {
			show_msg("Nâng cấp dữ liệu file config hoàn tất", "$theurl?step=delete");
		} else {
			show_msg('"config/config_global.php" Tập tin đã được cập nhật, "config/" chúng tôi không thể ghi vào thư mục chứa dữ liệu "data/" bạn hãy thiết lập lại quyền hạn hoặc Chmod dữ liệu ở máy chủ "config/" bao gồm tất cả thư mục.<br /><br /><a href="'.$theurl.'?step=delete">当您完成上述操作后点击这里继续</a>');
		}
	}
	...
	if(!$devmode) {
		show_msg("Đang xử lí dữ liệu.Bấm vào đây để đến với bước tiếp theo", "$theurl?step=style");
	}
	...
	if($deltables) {
		...
		$deltablehtml .= '</table>';
		echo "<p>Các dữ liệu sau đây không cần thiết sử dụng bạn có muốn xóa nó? </p>$deltablehtml";
	}
	...
	if($delcolumns) {
		...
		echo "<p>Các dữ liệu sau đây không cần thiết sử dụng bạn có muốn xóa nó? </p>$delcolumnhtml";
	}

	if(empty($deltables) && empty($delcolumns)) {
		echo "<p>Bạn không có nhu cầu xóa bảng dữ liệu nào.</p><a href=\"$theurl?step=style".($_GET['from'] ? '&from='.rawurlencode($_GET['from']).'&frommd5='.rawurlencode($_GET['frommd5']) : '')."\">Vui lòng click để đến với bước tiếp theo.</a></p>";
	} else {
		echo "<p><input type=\"submit\" name=\"delsubmit\" value=\"Submit Removal\"></p><p>Bạn cũng có thể bỏ qua các bảng dữ liệu<br><a href=\"$theurl?step=style".($_GET['from'] ? '&from='.rawurlencode($_GET['from']).'&frommd5='.rawurlencode($_GET['frommd5']) : '')."\">Tiếp theo.</a></p>";
	}
	echo '</form>';
	...
	if(empty($_GET['confirm'])) {
		show_msg("Bạn có muốn phục hồi lại giao diện mặc định?<br /><br /><a href=\"$theurl?step=style&confirm=yes".($_GET['from'] ? '&from='.rawurlencode($_GET['from']).'&frommd5='.rawurlencode($_GET['frommd5']) : '')."\">[ Có ]</a>&nbsp;&nbsp;<a href=\"$theurl?step=cache".($_GET['from'] ? '&from='.rawurlencode($_GET['from']).'&frommd5='.rawurlencode($_GET['frommd5']) : '')."\">[ Không ]</a>", '');
	}
	...
	show_msg("Giao diện mặc định đã được phục hồi, đang cập nhật cache vui lòng chờ đợi.", "$theurl?step=cache");
	...
	if($result == true) {
		$opensoso = '<br><br>Lời khuyên:<br>hãy sử dụng công cụ tìm kiểm trong admincp.<br>Từ khóa: <a href=\\\'../admin.php?frames=yes&action=cloud&operation=search\\\' target=\\\'_blank\\\'>Dữ liệu-&gt;Điện toán đám mây-&gt</a>.';
	}
	...
	if($_GET['from']) {
		show_msg('<span id="finalmsg">Cập nhật bộ nhớ cache,xin vui lòng chờ trong giây lát... ...</span><iframe src="../misc.php?mod=initsys" style="display:none;" onload="window.location.href=\''.$_GET['from'].'\'"></iframe>');
	} else {
		show_msg('<span id="finalmsg">Cập nhật bộ nhớ cache,xin vui lòng chờ trong giây lát... ...</span><iframe src="../misc.php?mod=initsys" style="display:none;" onload="document.getElementById(\'finalmsg\').innerHTML = \'Quá trình nâng cấp dữ liệu hoàn tất, vui lòng xóa file update.php này để bảo mật dữ liệu.'.$opensoso.'\'"></iframe>');
	}
...
function show_msg($message, $url_forward='', $time = 1, $noexit = 0, $notice = '') {
	...
	if($url_forward) {
		...
		$message = "<a href=\"$url_forward\">$message (Tiếp theo...)</a><br>$notice<script>setTimeout(\"window.location.href ='$url_forward';\", $time);</script>";
	}
...
function show_header() {
	...
	<title> Quá trình nâng cấp cơ sở dữ liệu </title>
	<style type="text/css">
	* {font-size:12px; font-family: Verdana, Arial, Helvetica, sans-serif; line-height: 1.5em; word-break: break-all; }
	...
	<body>
	<div class="bodydiv">
	<h1>Tiến trình nâng cấp cơ sở dữ liệu</h1>
	<div style="width:90%;margin:0 auto;">
	<table id="menu">
	<tr>
	<td{$nowarr[start]}>Bắt đầu update</td>
	<td{$nowarr[sql]}>Cập nhật cấu trúc SQL</td>
	<td{$nowarr[data]}>Cập nhật dữ liệu</td>
	<td{$nowarr[delete]}>Xóa cấu trúc SQL</td>
	<td{$nowarr[cache]}>Hoàn thành quá trình Update</td>
	</tr>
	</table>
	<br>
END;
...
<div id="footer">&copy; Tranlate by discuzvn.com 2012-2013 http://www.discuzvn.com</div>
	</div>
	<br>
	</body>
	</html>
