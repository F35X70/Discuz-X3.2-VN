	...
	converthtml : function() {
		...
		if(lastpage) {
			...
			for(var i=1; i<=lastpage; i++) {
				selector += '<option value="'+i+'" '+ (i == curpage ? 'selected' : '') +'>Đầu tiên'+i+'Trang</option>';
			}
			selector += '</select>';
			selector += '<span>Đầu tiên'+curpage+'Trang</span>';
		}

		$('div.pg').removeClass('pg').addClass('page').html('<a href="'+ prevpagehref +'">Trước</a>'+ selector +'<a href="'+ nextpagehref +'">Sau</a>');
...
var popup = {
	...
	open : function(pop, type, url) {
		...
		if(typeof pop == 'string') {
			$('#ntcmsg').remove();
			if(type == 'alert') {
				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><input class="button2" type="button" value="Xem" onclick="popup.close();"></dd></div>'
			} else if(type == 'confirm') {
				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><input class="redirect button2" type="button" value="Xem" href="'+ url +'"><a href="javascript:;" onclick="popup.close();">Hủy bỏ</a></dd></div>'
			}
...
var geo = {
	...
	locationerror : function(error) {
		...
		switch(error.code) {
			case error.TIMEOUT:
				geo.errmsg = "Lỗi thời gian. Vui lòng chờ trong giây lát.";
				break;
			case error.POSITION_UNAVAILABLE:
				geo.errmsg = 'Không thể phát hiện vị trí hiện tại của bạn';
			    break;
		    case error.PERMISSION_DENIED:
		        geo.errmsg = 'Xin vui lòng cho phép truy cập đến vị trí hiện tại của bạn';
		        break;
		    case error.UNKNOWN_ERROR:
		        geo.errmsg = 'Xảy ra một lỗi không rõ';
		        break;
		}
	},
			...
			if(status == true) {
				...
				if(pullheight >= 0 && pullheight < 150) {
					...
					if(!reloadflag) {
						...
						contentobj.find('#refreshtxt').html('F5 để làm mới.');
					}
					reloadflag = true;
				}
			}
