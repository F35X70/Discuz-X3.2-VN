...
function extrafunc_atListMenu(tag, op) {
	...
	if(!op) {
		...
		return 'Điền username:<br /><input type="text" id="atkeyword" style="width:240px" value="" class="px" onkeydown="atEnter(event, \'atListSet\')" onkeyup="atFilter(this.value, \'at_list\',\'atListSet\',event, true);" /><div class="p_pop" id="at_list" style="width:250px;"><ul><li>@Username để tag người bạn muốn vào đọc</li></ul></div>';
	} else {
		if($('atkeyword').value) {
...
function atFilter(kw, id, call, e, nae) {
	...
	if(nae || !atEnter(e, call)) {
		...
		if(atResult.length) {
			...
			$(id).innerHTML = '<ul>' + newlist + '<li class="xg1">@tài khoản, bạn có thể nhắc họ xem bài viết</li></ul>';
		} else {
