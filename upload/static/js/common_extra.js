...
function _checksec(type, idhash, showmsg, recall, modid) {
	...
	x.get('misc.php?mod=sec' + type + '&action=check&inajax=1&modid=' + modid + '&idhash=' + idhash + '&secverify=' + (BROWSER.ie && document.charset == 'utf-8' ? encodeURIComponent(secverify) : secverify), function(s){
		...
		if(s.substr(0, 7) == 'succeed') {
			...
			if(showmsg) {
				if(type == 'code') {
					showError('Mã xác nhận lỗi, xin vui lòng điền lại');
				} else if(type == 'qaa') {
					showError('Lỗi xác nhận, xin vui lòng điền lại');
				}
				recall(0);
			}
		}
...
function _copycode(obj) {
	...
	setCopy(BROWSER.ie ? obj.innerText.replace(/\r\n\r\n/g, '\r\n') : obj.textContent, 'Mã đã được sao chép vào clipboard');
}
...
function _setCopy(text, msg){
	if(BROWSER.ie) {
		...
		if(r) {
			...
			showDialog('<div class="c"><div style="width: 200px; text-align: center;">Cần "Cho phép truy cập"</div></div>', 'alert');
		}
	} else {
		var msg = '<div class="c"><div style="width: 200px; text-align: center; text-decoration:underline;">Nhấn vào đây để copy</div>' +
...
function _setShortcut() {
	$('shortcuttip').onclick = function() {
		var msg = '1. Click"' + '<a href="javascript:;" class="xi2 xw1" ';
		msg += 'onclick="this.href = \'forum.php?mod=misc&action=shortcut\';this.click();saveUserdata(\'setshortcut\', 1);"';
		msg += '>Download desktop shortcut</a>' + '" After the download is complete, you can move files to desktop<br />';
		msg += '2. Click"' + '<a href="forum.php?mod=misc&action=shortcut&type=ico" class="xi2 xw1">';
		msg += 'Download ICO icon</a>' + '"After the download is complete, right-click on the desktop shortcut file -> Properties -> Change Icon, and select the downloaded ICO icon';
		showDialog(msg, 'notice', 'Adding a desktop shortcut');
	};
