	...
	x.getJSON(url+'&_makehtml&r='+(+ new Date()), function(ret){
		...
		if(ret && (ret=ret['data']) && ret['status'] == 'html_ok') {
			if(obj) {
				...
				obj.innerHTML = '<a href="'+ret['path']+'" target="_blank">'+title+'Tạo khu mua sắm thành công.</a>';
			}
			if(ret['nexturl']) {
				if(obj) {
					obj.style.color = 'green';
					obj.innerHTML = 'Tạo mới'+title+(Math.round((ret['current']/ret['count'])*100))+'%';
				}
			...
			if(obj) {
				obj.style.color = 'red';
				obj.innerHTML = title+'Tạo mới không thành công';
			}
		}
	});
...
function make_html_batch(url, ids, callback, dom, single) {
	...
	if(this.single) {
		...
		progress_bar(cent);
		child.innerHTML = 'Đang tạo '+this.dom.getAttribute('mktitle');
		this.dom.innerHTML = '';
		this.dom.appendChild(child);
		this.make(id, child);
		this.child = child;
		var child2 = document.createElement('div');
		child2.innerHTML = '<a href="javascript:void(0);" id="mk_goon">Nếu trình duyệt của bạn không thể tự động chuyển, vui lòng nhấn vào đây...</a>';
	...
	make : function (id, child) {
		...
		x.getJSON(this.url+id+'&_makehtml&r='+(+ new Date()), function(ret){
			...
			if(obj.single) {
				...
				child.innerHTML = '<div class="mk_msg">'+'<a href="'+data['path']+'" target="_blank">'+obj.dom.getAttribute('mktitle')+'</a>Đã hoàn tất'+'</div>';
				if(obj.callback) {
				...
				progress_bar(cent);
				var str = 'Đã tạo ra '+obj.count+''+obj.dom.getAttribute('mktitle')+'và tạo ra'+obj.makedcount+'，';
				if(cent != 100) {
					child.innerHTML = str+'Là thành viên đầu tiên tạo '+current+' đã hoàn thành'+cent+'%';
				} else {
					child.style.color = 'blue';
					child.innerHTML = str+obj.dom.getAttribute('mktitle')+'đã hoàn tất.';
				}
				if(id) {
					obj.make(id, child);
