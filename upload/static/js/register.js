...
var lastusername = '', lastpassword = '', lastemail = '', lastinvitecode = '', stmp = new Array(), modifypwd = false, profileTips = 'If you do not need to change the password here, please leave blank';
...
function checkPwdComplexity(firstObj, secondObj, modify) {
	...
	firstObj.onblur = function () {
		if(firstObj.value == '') {
			var pwmsg = !modifypwd ? 'Hãy điền mật khẩu' : profileTips;
			if(pwlength > 0) {
	...
	firstObj.onkeyup = function () {
		if(pwlength == 0 || $(firstObj.id).value.length >= pwlength) {
			var passlevels = new Array('','Yếu','Tốt','Mạnh');
