...
drag.extend({
	...
	openStyleEdit : function (e) {
		...
		table += '<tr><th>Kiểu chữ</th><td><input type="text" id="fontsize" class="px p_fre vm" value="'+fontsize+'" size="2" />px <input type="text" id="fontcolor" class="px p_fre vm" value="'+fontcolor+'" size="2" />';
		...
		table += '<li class="bordera mtn"><label>Phải</label><select class="ps vm" id="bdrwidth" ><option value="">Size</option>'+this.getOption(widtharr,bdrwidth)+'</select>';
		...
		table += '<li class="bordera mtn"><label>Trái</label><select class="ps vm" id="bdlwidth" ><option value="">Size</option>'+this.getOption(widtharr,bdlwidth)+'</select>';
		...
		var f = '<p class="o pns"><button onclick="drag.saveStyle(\''+id+'\');drag.closeStyleEdit(\''+id+'\');" class="pn pnc" value="true">\n\
			<strong>Comfirm</strong></button><button onclick="drag.closeStyleEdit(\''+id+'\')" class="pn" value="true"><strong>Hủy bỏ</strong></button></p>';
		this.removeMenu(e);
	...
	createObj : function (e,objType,contentType) {
		if (objType == 'block' && !this.checkHasFrame()) {alert("Chú ý: Khung không được tìm thấy, thì bạn có thể thêm một khung.");spaceDiy.getdiy('frame');return false;}
		e = Util.event(e);
